...
noted earlier. Calculation scales as n^2/2. Using the data we trained our models on, the results were not very stable.
Furthermore, calculation time is not practical for a real-time location as the overhead time consumption for computation
is too great. Visualized in the plot of actuals and predicted values in Fig. 4, below, there are extreme outliers beyond
the boundaries of the building walls. The high volume of extreme values is not practical.
...
suited alternative to the K-Nearest Neighbors model used. Polynomial regression has a much smaller time complexity than K-NNsince it does not have to re-evaluate the entire data set with each added observation.
