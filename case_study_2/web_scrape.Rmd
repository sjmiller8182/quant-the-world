...
# Results
As visualized in Fig. 2, Results from analyzing geographic runner information identified that the nations with the fastest run times, represented by average, were from Tanzania, Morocco, Ethiopia, Kenya and Mexico. The nations with the slowest average run times were Ireland, Slovenia, Lebanon, Republic of Korea, and Jordan.

# Conclusion

## Trends over time

The Cherry Blossom race has experienced growth since the year 2000 as indicated by the linear trend line in the plot below. 
```{r, include = T}
...
runTime_lm = lm(year ~ runTime, data = runTime_by_year)
library(ggplot2)
p = ggplot(participation, aes(Group.1, runTime)) + 
  labs(title = 'Cherry Blossom Race Participation', caption = 'Figure 2: Cherry Blossom race participation with linear trend line') +
...
The average race completion time for all ages and genders has also experienced a positive trend. The correlation between race participation and the average race completion time is 92.78%.

```{r, include = T}
p = ggplot(runTime_by_year, aes(year, runTime)) + 
  labs(title = 'Average Run Time 1999 - 2012', caption = 'Figure 1 : Average race finish time with linear trend line') +
...
As visualized in Fig. 3, the states represented by the fastest overall runners - based on average race completion time - are Nevada, Colorado, Delaware, Idaho, and New Mexico. The states with the slowest average race completion times are North Dakota, Kentucky, Hawaii, Oklahoma, and Oregon.
