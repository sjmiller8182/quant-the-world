---
title: "Cherry Blossom Race Web Scraping"
author: "Justin Howard, Stuart Miller, and Paul Adams"
...
In this case study we collect data on participates of the Cherry Blossom Race
 from the Cherry Blossom Race website\footnotemark.
We find that there are a number of issues present in the data,
 including inconssitent web addressing formatting, inconsistent formatting, and missing data.
...
In this case study, we collected data on participants in the Cherry Blossom Race.
We scraped the data for Men and Woman from 1999 to 2012.
The data was stored under the base address in the following form
 where year is between 1999 and 2012 and page is a specific page name.

```
http://www.cherryblossom.org/results/<year>/<page>
```

The specific addresses where the data is stored are given in table 1.
While the structure follows `results/<year>` consistently, 
 the page naming for each year is not consistent.
In some cases, the names for the men's and women's pages are `men` and `women`, repectively.
While, in other cases, a code is used such as `cb99m` and `cb99f` in 1999 for `men` and `women` repectively.
