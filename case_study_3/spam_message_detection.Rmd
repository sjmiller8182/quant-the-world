...
Five example engineered features are given below (the full list is provided in the appendix).
...
The distributions of log-likelihoods for each class can be inspected in fig. 2 (box-plots). Based on distribution spread of log
...
```{r, spam_box_plot, echo=F, warning=F, fig.cap="Figure 1: NB Log Likelihood Ratios Distributions", fig.height=3.5}
...
```{r Error Types, fig.cap="Figure 2: NB Type I and Type II Error", echo=F, warning=F}
...
internal and external cross-validation. On internal 5-fold cross-validation, the Partition Tree scored an F1 of 0.957, while the Naive
...
The following features were engineered from the emails.
...
The following words were considered spam words for feature 19.
